...
Search the wallet provider for available accounts.

<Tabs>
  <TabItem value="discover-acc-use" label="Usage">
    ```js
    const discovered = await aioha.discoverAccounts(Providers.Ledger)
    ```
  </TabItem>
        ...
        "owner": [
          {
            "username": "randomvlogs",
            "path": "m/48'/13'/0'/0'/0'",
            "pubkey": "STM5FVEbhT4XkdGfvfnRDRgxTgpmbKyJjJ276Yz3U7s7YtNmkdHfo"
          }
        ],
        "active": [
          {
            "username": "randomvlogs",
            "path": "m/48'/13'/1'/0'/0'",
            "pubkey": "STM5jV2ZEoXVpi3MqTr2X7CwqxWvMdqAJJdfVCaQmyfcfmEHWHoqo"
          }
        ],
        "posting": [
          {
            "username": "randomvlogs",
            "path": "m/48'/13'/4'/0'/0'",
            "pubkey": "STM69dCa28Z78HjRUx2EhkoapsrXoXv111KzEadrxyEuWt8iiTC4K"
          }
        ]
      }
    }
...
<Tabs>
  ...
  :::warning
  As of v1.6.0-beta.4 release, there are no contracts deployed on the VSC mainnet. This method may not be up to date with the latest method to call a contract on mainnet.
  :::
  ```js
