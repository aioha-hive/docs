...
<Tabs>
  <TabItem value="enum-providers" label="Providers" default>
    ...
    The `LoginProvider` type (accepted by `login()`, `switchUser()`, etc.) also accepts any arbitrary string for [named custom providers](/docs/core/custom-provider#registering-custom-providers). This means you can pass `'my-custom-provider'` directly instead of going through the enum.
  </TabItem>
...
<Tabs>
  <TabItem value='login-use' label='Usage'>
    ...
    ```
    * `provider`: A `Providers` enum value or a [custom provider name](/docs/core/custom-provider#registering-custom-providers) string.
    * `msg`: Message to be signed
    * `keyType`: Key type to be used to sign the message. Not required for custom providers.
  ...
  <TabItem value='login-result' label='Result'>
    ...
    ```
    * `provider`: Name of provider selected by the user — a `Providers` enum value or a custom provider name string. See [Providers Enum](#enums) above for possibilities.
...
<Tabs>
  <TabItem value="logout-single" label='Current User'>
  ...
  <TabItem value="logout-all" label='All Users'>
...
```js
aioha.getProviders() // ['keychain', 'peakvault', 'ledger', 'hiveauth', 'hivesigner', 'beekeeper', ...]
```
