...
<Tabs>
  <TabItem value='config-keychain' label='Keychain'>
    * `loginTitle` *(optional)*: Title to be displayed on login
  </TabItem>
...
<Tabs>
  <TabItem value='login-ex' label='Example'>
    ...
    * `hiveauth.cbWait`: Callback function returning auth payload. It shall display a QR code using `payload` as data to be scanned by a HiveAuth PKSA.
  </TabItem>
  <TabItem value='login-result' label='Result'>
    ...
    ```
    * `provider`: Name of provider selected by the user. See [Providers Enum](#providers-enum) above for possibilities.
...
```js
aioha.getProviders() // ['keychain', 'ledger', 'hiveauth', 'hivesigner']
```
...
<Tabs>
  <TabItem value='vote-witness' label='Vote Witness'>
    ```js
    const voteWitness = await aioha.voteWitness('votee', true)
    const unvoteWitness = await aioha.voteWitness('votee', false)
    ```
  </TabItem>
