...
<Tabs>
  <TabItem value='stake' label='Stake'>
    <Admonition type="note">
      `to` is an optional parameter that is used when staking HIVE to another account.
    </Admonition>
    ```js
    {
...
The `sign_msg_request` event is emitted every time a message signature request has been made to the user's wallet  (excluding HiveAuth, see [below](#hiveauth-challenge-request)).
...
The `sign_tx_request` event is emitted every time a transaction signature request has been made to the user's wallet  (excluding HiveAuth, see [below](#hiveauth-sign-transaction-request)).
...
The `hiveauth_challenge_request` event is emitted every time a new HiveAuth message signature request is created.
...
The `hiveauth_sign_request` event is emitted every time a new HiveAuth transaction signature request is created.
