...
|Feature|Keychain|HiveAuth|HiveSigner|Ledger|
|-|-|-|-|-|
|Login|✅|✅|✅<sup>1</sup>|✅|
|Memo encryption|✅|❌|❌|❌|
|Memo decryption|✅|❌|✅<sup>2</sup>|❌|
|Sign message|✅|✅|❌|✅|
|Sign tx (without broadcast)|✅|❌<sup>3</sup>|❌|✅|
|Sign tx (with broadcast)|✅|✅|✅|✅|
|Transact using owner auth|❌|❌|✅|✅|

<sup>1</sup> A HiveSigner access token will be returned instead of an ECDSA signature.<br/><sup>2</sup> Memo must be encrypted using `@hivesigner`'s public posting key.<br/><sup>3</sup> While technically supported by the provider, the PKSA generates the transaction headers instead of using supplied values. This is unhelpful for signing multisig transactions.
